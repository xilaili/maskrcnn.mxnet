...
def demo_net(predictor, data, image_names):
    data = [[mx.nd.array(data[i][name]) for name in data_names] for i in xrange(len(data))]
    # warm up
    for i in xrange(2):
    ...
    for idx, im_name in enumerate(image_names):
        ...
        if not config.TEST.USE_MASK_MERGE:
            ...
            result_masks, result_dets = gpu_mask_voting(masks, boxes, scores[0], config.NUM_CLASSES,
...
def main():
    ...
    data, image_names = load_data()
    predictor = get_net(data, sym, args.prefix, args.epoch, ctx)
    demo_net(predictor, data, image_names)
