...
from rcnn.processing.bbox_transform import bbox_overlaps, bbox_transform, remove_repetition
from rcnn.processing.bbox_regression import expand_bbox_regression_targets
from rcnn.mask.mask_transform import intersect_box_mask
from rcnn.config import config
import cPickle


class ProposalAnnotatorOperator(mx.operator.CustomOp):
    def __init__(self, num_classes, mask_size, binary_thresh, batch_images, batch_rois, fg_fraction):
...
class ProposalAnnotatorProp(mx.operator.CustomOpProp):
    def __init__(self, num_classes, mask_size, binary_thresh, batch_images, batch_rois, fg_fraction='0.25'):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ProposalAnnotatorOperator(self._num_classes, self._mask_size, self._binary_thresh,
                                         self._batch_images, self._batch_rois, self._fg_fraction)
